data "aws_iam_policy_document" "analysis_vehicle_plate_history" {
  statement {
    actions = [
      "logs:CreateLogGroup",
...
module "lambda_analysis_vehicle_plate_history" {
  source        = "../modules/lambda"
  name          = "${var.project}-analysis-vehicle-plate-history"
  source_bucket = module.global_variables.source_bucket
  project       = var.project
  policy_json   = data.aws_iam_policy_document.analysis_vehicle_plate_history.json
  handler       = "src/controllers/${var.project}/send-request-analysis/vehicle/plate-history/index.handler"
