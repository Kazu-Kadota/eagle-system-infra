data "aws_iam_policy_document" "analysis_vehicle_second_driver" {
  statement {
    actions = [
      "logs:CreateLogGroup",
...
module "lambda_analysis_vehicle_second_driver" {
  source        = "../modules/lambda"
  name          = "${var.project}-analysis-vehicle-second-driver"
  source_bucket = module.global_variables.source_bucket
  project       = var.project
  policy_json   = data.aws_iam_policy_document.analysis_vehicle_second_driver.json
  handler       = "src/controllers/${var.project}/send-request-analysis/vehicle/second-driver/index.handler"
